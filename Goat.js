 ...
 * 
 * This version works exactly like the original, but with fbstate stored in Firebase.
 */
...
async function loginBot() {
  try {
    ...
    // Get bot info – exactly like original login.js
    try {
    ...
    // ===== START LISTENING (EXACTLY LIKE ORIGINAL) =====
...
// ===== LOAD COMMANDS (SAME AS ORIGINAL loadScripts) =====
...
async function startListening(api, threadsData, usersData, dashBoardData, globalData) {
  ...
  // Start listening – exactly like original login.js callback
  api.listenMqtt(async (err, event) => {
    ...
    // Handle events using the same logic as original
...
// ===== HANDLE EVENTS (MIRRORS ORIGINAL handlerAction) =====
async function handleEvent(api, event, threadsData, usersData, dashBoardData, globalData) {
  try {
    // Process event commands first (like original)
